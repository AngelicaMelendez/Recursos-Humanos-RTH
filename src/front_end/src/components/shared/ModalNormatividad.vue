  ...
  <div v-if="isOpen" class="modal-overlay">
    <div class="modal-container">
      ...
      <main class="modal-content" @scroll="manejarScroll">
        <p class="intro-text">
          Para poder proceder con su solicitud, es obligatorio revisar y aceptar las Normatividades y Lineamientos internos vigentes de Radio y Televisión de Hidalgo.
        </p>
        ...
        <div v-for="doc in documentos" :key="doc.id" class="document-item">
          ...
          <p><strong>Tipo:</strong> {{ doc.tipo }} | <strong>Publicación:</strong> {{ doc.fecha_publicacion }}</p>
          <a :href="`http://localhost:8000/${doc.archivo_pdf}`" target="_blank" class="view-link">
             Hacer clic aquí para leer la Normatividad
          </a>
        </div>
      ...
      <footer class="modal-footer">
        <label class="checkbox-container">
          ...
          <span :class="{ 'disabled-text': !completoScroll }">
            He leído, Comprendo y Acepto cumplir con los Lineamientos Institucionales establecidos.
          </span>
        </label>
...
watch(() => props.isOpen, (newVal) => {
  if (newVal) {
    ...
    completoScroll.value = false; // Cambiar a true si no quieres obligar a hacer scroll
  }
});

// Forzar a que lean hasta el fondo (experiencia de usuario profesional)
const manejarScroll = (e) => {
  const { scrollTop, scrollHeight, clientHeight } = e.target;
  // Margen de 5px de tolerancia para el final del scroll
